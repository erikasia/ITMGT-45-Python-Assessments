...
def tic_tac_toe(board):
    ...
    outcome = "THERE IS NO WINNER, PLAY AGAIN"
...
def eta(first_stop, second_stop, route_map):
    ...
    leg=next(key for key in route_map.keys() if key[0] == first_stop)

    while leg[1] !=second_stop:
        newtime += route_map[leg]['travel_time_minutes']
        leg= next(key for key in route_map.keys() if key[0] == leg [1])
